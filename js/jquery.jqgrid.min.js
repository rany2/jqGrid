...
wa=function(b,a){r&&"string"===typeof a&&(/^\w+$/.test(a)?ba[b]=C(a):/^\[\w+\]$/.test(a)&&(ba[b]=Pa(a.substring(1,a.length-1))))},Ka=function(b){var a=b.name,c=r?b.xmlmap||a:"local"===n&&!k.dataTypeOrg||"json"===n||"jsonp"===n?b.jsonmap||a:a;!1!==k.keyName&&!0===b.key&&(k.keyName=a);if("string"===typeof c||m(c))ba[a]=c;m(c)||wa(a,c)};for(t=0;t<D;t++)Ka(L[t]);D=ea.length;for(t=0;t<D;t++)I=ea[t],"object"===typeof I&&null!=I?Ka(I):wa(I,I);A=!1===k.keyName?m(q.id)?q.id.call(this,a):q.id:k.keyName;isNaN(A)?
m(A)||(M[A]&&(F=M[A].order),r&&("string"===typeof A&&/^\[\w+\]$/.test(A)?A=Pa(A.substring(1,A.length-1)):"string"===typeof A&&/^\w+$/.test(A)&&(A=C(A)))):F=Number(A);x=v(a,q.root,!0);if(q.row)if(1===x.length&&"string"===typeof q.row&&/^\w+$/.test(q.row)){D=[];H=x[0].childNodes;ja=H.length;for(I=0;I<ja;I++)Q=H[I],1===Q.nodeType&&Q.nodeName===q.row&&D.push(Q);x=D}else x=v(x,q.row,!0);null==x&&l(a)&&(x=a);x||(x=[]);a=x.length;0<a&&0>=k.page&&(k.page=1);Ka=parseInt(k.rowNum,10);f&&(Ka*=f+1);var L=[],
za=[],G,D=[];for(t=0;t<a;t++){G=x[t];w=q.repeatitems&&q.cell?v(G,q.cell,!0)||G:G;y=q.repeatitems&&(r||l(w));B={};W={};if(r&&!y&&null!=w)for(H=w.childNodes,ja=H.length,I=0;I<ja;I++)Q=H[I],1===Q.nodeType&&(W[Q.nodeName]=Q);for(E in M)M.hasOwnProperty(E)&&(I=M[E],y?(H=w[I.order],r&&null!=H&&(H=H.textContent||H.text)):H=null!=ba[E]&&"string"!==typeof ba[E]?ba[E](w):v(w,"string"===typeof ba[E]?ba[E]:I.name),1===I.type&&(I=ea[I.index],null!=I&&m(I.convert)&&(H=I.convert(H,w))),void 0!==H&&(B[E]=H));void 0!==
